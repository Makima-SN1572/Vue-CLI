...
const router = new VueRouter({
    mode:'hash',
    //mode:'hash'默认值是hash,就是我们路由路径的/#/.....  #+后面所有东西都统称为hash值,这部分不会去跟服务器要这里路径下的数据
   // mode:'history',//这里就不会有/#/,


    //hash与history的区别:1.一个带#一个不带,2.hash值兼容性好,history兼容性略差
    //history,项目部署后网络请求会去找服务器相应的路由组地址,要是服务器没有就直接404了,而hash值就不会出现这种情况,他不会去要求服务器请求hash值
